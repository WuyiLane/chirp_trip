...
| 随笔详情 | `pages/post/` | 图片轮播、折叠头、正文、评论（可发表）、相关推荐 |
...
11. **首页**：状态栏留白，「今日推荐」吸顶（毛玻璃条，卡片从底下滚过），滚到底上拉加载下一页；底栏是 iOS 26 式悬浮毛玻璃胶囊，四个 tab 图标 + 文字常显，选中态不加背景：图标线条像笔一样从头描一遍（按路径长度取段，900ms），描到 80% 时黄色开始渗进来、弹表情，文字变黑加粗 —— `pages/home/home_page.dart` / `widgets/chick_tab_bar.dart`
12. **发现 / 关注**：标题栏是毛玻璃，列表从底下滚过；两个 tab 装在 PageView 里，点标题或左右滑都能切 —— `pages/discover/discover_page.dart`

13. **评论**：底部「有想法就告诉TA哦」或面板底部那条 → 弹出跟着键盘走的输入框，发出去插到列表最前面、计数 +1；「查看所有 N 条评论」是微博那种浮动面板（拖把手下滑关闭、上拖吸到全屏） —— `widgets/drag_sheet.dart` / `pages/post/post_detail_page.dart`
