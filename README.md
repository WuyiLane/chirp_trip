...
11. **首页**：状态栏留白，「今日推荐」吸顶，滚到底上拉加载下一页；底栏是 iOS 26 式悬浮毛玻璃胶囊，四个 tab 图标 + 文字常显，选中态不加背景：图标线条先像笔一样从头描一遍（按路径长度取段，700ms），描完再填黄弹表情，文字变黑加粗；切走时线条反向擦掉再补成线框 —— `pages/home/home_page.dart` / `widgets/chick_tab_bar.dart`
